...
export default function PracticalTest({
  ...
  const subjectId = Array.isArray(_params.subjectId) ? _params.subjectId[0] : _params.subjectId;
  ...
  const [questionsList] = useState(() => {
    ...
    return getNavState("practicalTest")?.questions || [];
  });

  useEffect(() => {
    if (!questionsList.length) {
      router.push(`/test-instructions/${subjectId}`);
    }
  }, [questionsList.length, router, subjectId]);
