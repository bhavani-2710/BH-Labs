...
function buildJournalHtml({ subExp, experiment, codeText, outputText, referenceSolutionCode, flowchartDataUrl }) {
  ...
  if (isGuided) {
    ...
    guidedStepsBlocks = steps.map((st, i) => {
      ...
      const imgHtml = st.imageUrl || st.image
        ? `<img src="${st.imageUrl || st.image}" alt="Step ${stepNum} screenshot" class="step-img" />`
        : "";
...
<style id="__journal_pdf_styles__">
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .page {
    width: 794px;
    height: 1123px;
    padding: 56px 54px 70px 54px; /* top/right/bottom/left — bottom makes room for footer */
    background: #fff;
    position: relative;
...
export async function generateJournalPdf({ experiment, subPart = "a", codeText, outputText }) {
  ...
  container.innerHTML = styleHtml + `<div class="scratchpad" style="width:794px; padding: 56px 54px 70px 54px; box-sizing: border-box;">${contentHtml}</div>`;
  ...
  const PAGE_PADDING_T = 56;
  const PAGE_PADDING_B = 70;
  const BUDGET = A4_HEIGHT - PAGE_PADDING_T - PAGE_PADDING_B; // 997px
  ...
  pages.forEach((pageBlocks, index) => {
    ...
    pageDiv.style.cssText = "width:794px; height:1123px; padding: 56px 54px 70px 54px; background:#fff; position:relative; box-sizing:border-box; overflow:hidden;";
