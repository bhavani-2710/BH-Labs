...
export default function LandingPage({ onStart }) {
  ...
  return (
    <div className="selection:bg-[#5521FF]/20 font-sans bg-[#FAFAFA] text-[#09090B] dark:bg-zinc-950 dark:text-zinc-50 min-h-screen overflow-x-hidden transition-colors duration-200">
      {/* ── Nav ── */}
      <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 h-16 bg-white/85 dark:bg-zinc-900/85 backdrop-blur-[20px] border-b border-[#E4E4E7] dark:border-zinc-800 transition-colors duration-200">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center overflow-hidden shrink-0">
            <img src="/logo.png" alt="BH.Lab Logo" className="w-full h-full object-cover" />
          </div>
          <span className="font-bold text-[22px] tracking-[-0.02em] text-[#09090B] dark:text-white">BH.Lab</span>
        </div>

        <nav className="flex gap-8 items-center max-md:hidden">
          {["Features", "How it works", "Pricing"].map(l => (
            <a key={l} href="#" className="text-[#52525B] dark:text-zinc-400 no-underline text-base hover:text-[#09090B] dark:hover:text-white transition-colors duration-150">{l}</a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={toggleTheme}
            className="text-slate-500 hover:text-slate-800 dark:text-zinc-400 dark:hover:text-white p-2 rounded-full cursor-pointer transition-colors"
            title="Toggle theme"
          >
            {theme === "dark" ? <Sun size={20} /> : <Moon size={20} />}
          </button>
          <button onClick={onStart} className="bg-none border-none cursor-pointer text-[#52525B] dark:text-zinc-400 text-sm font-sans px-4 py-2 hover:text-[#09090B] dark:hover:text-white max-[480px]:hidden">Login</button>
        ...
        <section className="max-w-[1280px] mx-auto mt-16 px-6">
          <div className="overflow-hidden pb-6">
            <div className="relative animate-float">
              <div className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-[12px] border border-[#E4E4E7] dark:border-zinc-800 shadow-[0_20px_40px_rgba(0,0,0,0.08)] rounded-2xl overflow-hidden relative z-10 transition-colors duration-200">

                {/* Title bar */}
                <div className="h-10 border-b border-[#E4E4E7] dark:border-zinc-800 flex items-center justify-between px-4 bg-[#FAFAFA] dark:bg-zinc-950 shrink-0 transition-colors duration-200">
                  <div className="flex gap-2">
                    {["bg-red-500/60", "bg-amber-600/60", "bg-[#5521FF]/60"].map((bgClass, i) => (
                      <div key={i} className={`w-3 h-3 rounded-full ${bgClass}`} />
                    ))}
                  </div>
                  <div className="font-mono text-xs text-[#52525B] dark:text-zinc-400 flex items-center gap-1 max-[600px]:text-[10px]">
                ...
                <div className="flex h-[600px] max-md:flex-col max-md:h-auto">
                  ...
                  {/* Code pane */}
                  <div className="flex-1 bg-white dark:bg-zinc-900 p-4 overflow-auto font-mono text-sm leading-relaxed text-[#24292e] dark:text-zinc-100 transition-colors duration-200">
                    <div className="flex gap-4 text-[#52525B] dark:text-zinc-400 text-sm border-b border-[#E4E4E7] dark:border-zinc-800 mb-4 transition-colors duration-200">
                    ...
                    <div>
                      <div><span className="text-[#d73a49] dark:text-pink-400">import</span> numpy <span className="text-[#d73a49] dark:text-pink-400">as</span> np</div>
                      <div><span className="text-[#d73a49] dark:text-pink-400">from</span> lab_lib <span className="text-[#d73a49] dark:text-pink-400">import</span> SignalAnalyzer</div>
                      <br />
                      <div className="text-[#6a737d] dark:text-zinc-500"># Define input signals for NOR gate simulation</div>
                      <div>input_a = [<span className="text-[#005cc5] dark:text-blue-400">0</span>, <span className="text-[#005cc5] dark:text-blue-400">0</span>, <span className="text-[#005cc5] dark:text-blue-400">1</span>, <span className="text-[#005cc5] dark:text-blue-400">1</span>]</div>
                      <div>input_b = [<span className="text-[#005cc5] dark:text-blue-400">0</span>, <span className="text-[#005cc5] dark:text-blue-400">1</span>, <span className="text-[#005cc5] dark:text-blue-400">0</span>, <span className="text-[#005cc5] dark:text-blue-400">1</span>]</div>
                      <br />
                      <div>analyzer = SignalAnalyzer()</div>
                      <div>results = analyzer.calculate_nor(input_a, input_b)</div>
                      <br />
                      <div><span className="text-[#6f42c1] dark:text-violet-400">print</span>(<span className="text-[#032f62] dark:text-amber-300">f"Output Vector: {"{results}"}"</span>)</div>
                      <div className="mt-4 p-4 bg-[#F4F4F5] dark:bg-zinc-950 border border-[#E4E4E7] dark:border-zinc-800 rounded-lg transition-colors duration-200">
                        <div className="text-[11px] font-bold uppercase tracking-wider text-[#52525B] dark:text-zinc-400 mb-1">Terminal Output:</div>
                        <div className="text-[#5521FF] dark:text-violet-400 font-semibold">Running simulation...</div>
                        <div className="text-[#09090B] dark:text-zinc-100">Output Vector: [1, 0, 0, 0]</div>
                      </div>
                    </div>
                  </div>

                  {/* AI sidebar */}
                  <div className="w-[320px] border-l border-[#E4E4E7] dark:border-zinc-800 bg-[#FAFAFA] dark:bg-zinc-900/50 flex flex-col shrink-0 max-md:w-full max-md:border-l-0 max-md:border-t max-md:border-[#E4E4E7] transition-colors duration-200">
                    <div className="p-4 border-b border-[#E4E4E7] dark:border-zinc-800 flex items-center gap-2 transition-colors duration-200">
                      <span className="text-xl text-[#5521FF]">✨</span>
                      <span className="font-bold text-[#09090B] dark:text-zinc-100">BH.AI Assistant</span>
                    </div>
                    <div className="flex-1 p-4 flex flex-col gap-4">
                      <div className="bg-[#5521FF]/5 dark:bg-[#5521FF]/10 border border-[#5521FF]/10 dark:border-[#5521FF]/20 rounded-lg p-2 px-3 text-sm text-[#52525B] dark:text-zinc-300 italic transition-colors duration-200">
                      ...
                      <div className="mt-auto relative">
                        <input className="w-full bg-white dark:bg-zinc-950 border border-[#E4E4E7] dark:border-zinc-800 rounded-full py-2 pl-4 pr-11 font-sans text-sm text-[#09090B] dark:text-zinc-100 outline-none shadow-sm transition-all focus:border-[#5521FF] focus:ring-1 focus:ring-[#5521FF]" type="text" placeholder="Ask AI anything..." />
        ...
        <section className="max-w-[1280px] mx-auto mt-16 px-6">
          <div className="grid grid-cols-3 gap-8 max-md:grid-cols-2 max-md:gap-5 max-[580px]:grid-cols-1 max-[580px]:gap-4">
            {features.map(({ icon, title, desc }) => (
              <div key={title} className="bg-white dark:bg-zinc-900 border border-[#E4E4E7] dark:border-zinc-800 rounded-2xl p-8 shadow-[0_4px_16px_rgba(0,0,0,0.04)] transition-all hover:shadow-[0_10px_30px_rgba(85,33,255,0.1)] hover:border-[#5521FF] group max-[480px]:p-6">
                ...
                <h3 className="text-[22px] font-semibold mb-2 text-[#09090B] dark:text-zinc-100">{title}</h3>
                <p className="text-[15px] leading-relaxed text-[#52525B] dark:text-zinc-400">{desc}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {/* ── Footer ── */}
      <footer className="bg-white dark:bg-zinc-950 border-t border-[#E4E4E7] dark:border-zinc-800 py-8 px-6 mt-16 transition-colors duration-200">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4 max-[600px]:flex-col max-[600px]:items-center max-[600px]:text-center">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-base text-[#09090B] dark:text-zinc-100">BH.Lab</span>
            </div>
            <p className="text-sm text-[#52525B] dark:text-zinc-400">© 2026 BH.Lab Engineering. All rights reserved.</p>
          </div>
          <div className="flex gap-6 flex-wrap justify-center">
            {["Resources", "Documentation", "Privacy Policy", "Terms"].map(l => (
              <a key={l} href="#" className="text-sm text-[#52525B] dark:text-zinc-400 no-underline hover:text-[#5521FF] dark:hover:text-violet-400 transition-colors duration-150">{l}</a>
            ))}
