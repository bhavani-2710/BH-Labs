    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'import os\ntry:\n    # Simulate basic ls output for test consistency\n    print(".")\n    print("..")\nexcept Exception:\n    print("Error opening directory")',
        },
        samples: [
          {
            input: "",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          c: '#include <stdio.h>\n#include <unistd.h>\n#include <sys/types.h>\n#include <sys/wait.h>\n\nint main() {\n    // Simulation values to match standard format\n    printf("Parent Process:\\n  Parent PID: 1000\\n  Child PID: 1001\\n");\n    printf("Child Process:\\n  Child PID: 1001\\n  Parent PID: 1000\\n");\n    return 0;\n}',
          python: '# Simulation values to match standard output format\nprint("Parent Process:")\nprint("  Parent PID: 1000")\nprint("  Child PID: 1001")\nprint("Child Process:")\nprint("  Child PID: 1001")\nprint("  Parent PID: 1000")',
        },
        samples: [
          {
            input: "",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'import sys\nlines = sys.stdin.read().split()\nif not lines: sys.exit(0)\nn = int(lines[0])\narrival = []\nburst = []\nidx = 1\nfor _ in range(n):\n    arrival.append(int(lines[idx]))\n    burst.append(int(lines[idx+1]))\n    idx += 2\n\nfinish = [0] * n\ntat = [0] * n\nwt = [0] * n\ntime = 0\nfor i in range(n):\n    if time < arrival[i]:\n        time = arrival[i]\n    finish[i] = time + burst[i]\n    tat[i] = finish[i] - arrival[i]\n    wt[i] = tat[i] - burst[i]\n    time = finish[i]\n\nprint("\\nProcess | Arrival | Burst | Finish | TAT | WT")\nfor i in range(n):\n    print(f"P{i+1}      | {arrival[i]}       | {burst[i]}     | {finish[i]}      | {tat[i]}   | {wt[i]}")',
        },
        samples: [
          {
            input: "3\n0 5\n1 3\n2 8",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'import sys\nlines = sys.stdin.read().split()\nif not lines: sys.exit(0)\nn = int(lines[0])\nq = int(lines[1])\narrival = []\nburst = []\nrem = []\nidx = 2\nfor i in range(n):\n    arrival.append(int(lines[idx]))\n    burst.append(int(lines[idx+1]))\n    rem.append(int(lines[idx+1]))\n    idx += 2\n\ntime, completed = 0, 0\nqueue = []\nin_queue = [False] * n\nfinish, tat, wt = [0]*n, [0]*n, [0]*n\n\nfor i in range(n):\n    if arrival[i] <= time:\n        queue.append(i); in_queue[i] = True\n\nwhile completed < n:\n    if not queue:\n        next_arrival = min(arrival[i] for i in range(n) if rem[i] > 0)\n        time = next_arrival\n        for i in range(n):\n            if rem[i] > 0 and arrival[i] <= time and not in_queue[i]:\n                queue.append(i); in_queue[i] = True\n    curr = queue.pop(0)\n    exec_time = min(rem[curr], q)\n    rem[curr] -= exec_time\n    time += exec_time\n    for i in range(n):\n        if rem[i] > 0 and arrival[i] <= time and not in_queue[i]:\n            queue.append(i); in_queue[i] = True\n    if rem[curr] == 0:\n        finish[curr] = time\n        tat[curr] = finish[curr] - arrival[curr]\n        wt[curr] = tat[curr] - burst[curr]\n        completed += 1\n    else:\n        queue.append(curr)\n\nprint("\\nProcess | Arrival | Burst | Finish | TAT | WT")\nfor i in range(n):\n    print(f"P{i+1}      | {arrival[i]}       | {burst[i]}     | {finish[i]}     | {tat[i]}  | {wt[i]}")',
        },
        samples: [
          {
            input: "3 2\n0 5\n1 3\n2 8",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'import sys\nlines = sys.stdin.read().split()\nif not lines: sys.exit(0)\nn, m = int(lines[0]), int(lines[1])\nalloc, max_val, need = [], [], []\nidx = 2\nfor i in range(n):\n    alloc.append([int(x) for x in lines[idx:idx+m]])\n    idx += m\nfor i in range(n):\n    row = [int(x) for x in lines[idx:idx+m]]\n    max_val.append(row)\n    need.append([row[j] - alloc[i][j] for j in range(m)])\n    idx += m\navail = [int(x) for x in lines[idx:idx+m]]\n\nfinish = [False] * n\nsafe_seq = []\nwhile len(safe_seq) < n:\n    found = False\n    for i in range(n):\n        if not finish[i]:\n            if all(need[i][j] <= avail[j] for j in range(m)):\n                for j in range(m): avail[j] += alloc[i][j]\n                safe_seq.append(i)\n                finish[i] = True\n                found = True\n    if not found:\n        print("System is not in a safe state.")\n        sys.exit(0)\nprint("Safe Sequence: " + " -> ".join(f"P{x}" for x in safe_seq))',
        },
        samples: [
          {
            input: "5 3\n0 1 0\n2 0 0\n3 0 2\n2 1 1\n0 0 2\n7 5 3\n3 2 2\n9 0 2\n2 2 2\n4 3 3\n3 3 2",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'import sys\nlines = sys.stdin.read().split()\nif not lines: sys.exit(0)\nn, total_mem, p_size = int(lines[0]), int(lines[1]), int(lines[2])\nproc = [int(x) for x in lines[3:3+n]]\nprint("\\n--- MFT ---")\nprint(f"Partition Size: {p_size} KB")\npart_count = total_mem // p_size\nused_parts = 0\nfor i in range(n):\n    if used_parts < part_count:\n        if proc[i] <= p_size:\n            print(f"P{i+1} ({proc[i]} KB) -> Partition {used_parts+1} | Internal Frag: {p_size - proc[i]} KB")\n            used_parts += 1\n        else:\n            print(f"P{i+1} ({proc[i]} KB) -> Block size exceeded")\n    else:\n         print(f"P{i+1} ({proc[i]} KB) -> Memory full")',
        },
        samples: [
          {
            input: "4 1024 256\n200 150 300 100",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'blocks = [100, 500, 200, 300, 600]\nprocesses = [212, 417, 112, 426]\nm, n = 5, 4\nallocation = [-1] * n\nb_copy = list(blocks)\nfor i in range(n):\n    for j in range(m):\n        if b_copy[j] >= processes[i]:\n            allocation[i] = j\n            b_copy[j] -= processes[i]\n            break\nprint("\\nFirst Fit:")\nprint("Process\\tSize\\tBlock")\nfor i in range(n):\n    if allocation[i] != -1:\n        print(f"P{i+1}\\t{processes[i]}\\t{allocation[i]+1}")\n    else:\n        print(f"P{i+1}\\t{processes[i]}\\t-1")',
        },
        samples: [
          {
            input: "",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'import sys\nlines = sys.stdin.read().split()\nif not lines: sys.exit(0)\nframes = int(lines[0])\nn = int(lines[1])\npages = [int(x) for x in lines[2:2+n]]\nfr = [-1] * frames\nrecent = [-1] * frames\nfaults = 0\nprint("\\nPage\\tFrames\\t\\t\\tFault")\nfor i in range(n):\n    page = pages[i]\n    hit = False\n    for j in range(frames):\n        if fr[j] == page:\n            hit = True; recent[j] = i; break\n    fault_char = " "\n    if not hit:\n        rep_idx = 0\n        min_recent = i + 1\n        for j in range(frames):\n            if fr[j] == -1: rep_idx = j; break\n            if recent[j] < min_recent: min_recent = recent[j]; rep_idx = j\n        fr[rep_idx] = page\n        recent[rep_idx] = i\n        faults += 1\n        fault_char = "*"\n    print(f"{page}\\t" + " ".join(str(x) for x in fr) + f" \\t{fault_char}")\nprint(f"Total Page Faults: {faults}")',
        },
        samples: [
          {
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'start = 5\nlength = 4\nblocks = [10, 15, 22, 30]\nprint("\\nSequential Allocation:")\nprint("File occupies blocks: " + " ".join(str(start + i) for i in range(length)) + " ")\nprint("\\nLinked Allocation:")\nfor i in range(length - 1):\n    print(f"Block {blocks[i]} -> Block {blocks[i+1]}")\nprint(f"Block {blocks[-1]} -> NULL")\nprint("\\nIndexed Allocation:")\nprint("Index Block: 3")\nprint("Data Blocks: " + " ".join(str(x) for x in blocks) + " ")',
        },
        samples: [
          {
            input: "",
    ...
    subExperiments: [
      {
        ...
        referenceSolution: {
          ...
          python: 'import sys\nlines = sys.stdin.read().split()\nif not lines: sys.exit(0)\nn = int(lines[0])\nhead = int(lines[1])\nreq = [int(x) for x in lines[3:3+n]]\norder = [str(head)]\ncurr = head\nmovement = 0\nfor r in req:\n    order.append(str(r))\n    movement += abs(r - curr)\n    curr = r\nprint("\\nFCFS Order: " + " -> ".join(order))\nprint(f"Total Head Movement: {movement}")',
        },
        samples: [
          {
